...
<body>
 ...
 <tbody style="font-size: 20px;">
   ...
   <td align="center"><b><br>DATA PRODUK</b>
...
.tulisan_lima{
	...
	if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter
		...
		if($filter == '1'){ // Jika filter nya 1 (per tanggal)
			...
			echo '<b>Data Produk Tanggal '.$tanggal.'</b><br /><br />';
			...
			$query = "SELECT * FROM produk WHERE YEAR(tanggal)='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
		}
	}else{ // Jika user tidak memilih filter
		echo '<b>Semua Data Transaksi</b><br /><br />';

		$query = "SELECT * FROM produk ORDER BY tanggal"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
	}
	?>
